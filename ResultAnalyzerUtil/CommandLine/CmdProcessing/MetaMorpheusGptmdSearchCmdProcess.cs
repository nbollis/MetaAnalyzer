...
public class MetaMorpheusSearchCmdProcess : MetaMorpheusCmdProcess
{
    public bool IsLibraryCreator { get; }
    ...
    /// For use in the creation of the internal MM comparison searches
    /// </summary>
    public MetaMorpheusSearchCmdProcess(string[] spectraPaths, string[] dbPaths, string search, string outputPath,
        string summaryText, double weight, string workingDir, string? quickName = null, string programExe = "CMD.exe")
        : base(spectraPaths, dbPaths, [search], outputPath, summaryText, weight, workingDir, quickName, programExe)
    {
        IsLibraryCreator = search.Contains("Build");
    }
